...
/**
 *BtSMS class defines the interaction 
 *with the phone on BLUETOOTH.   
 * 
...
public class BtSMS implements btInterface{
	...
	//creating Rfcomm socket and connection
	...
	private String sendAtcommands(String commands) throws IOException, InterruptedException{
		...
		//intermediate state storage array messByte
		byte[] tmpByte = new byte[0];
		//stores the response to the socket
		byte[] messByte = new byte[0];
		int bytesRead = 0;
		int bytesReadAdd = 0;
		String mess = "";
		//send AT command to telephone
		...
		if(!commands.contains("AT+CMGS") && !commands.contains("AT+CMGD")){
			while(true){
					...
					//increase the size of the array on bytesRead
					messByte = new byte[tmpByte.length + bytesRead];
					
					//copying bytes read to the end messByte
					...
					// copy the contents tmpByte in the start of the array messByte
	...
	//verification of support phone reading sms on BLUETOOTH
	...
	public void readSMS(String bankMemory){
		...
		try {
			...
			//notify the main thread the maximum number of sms
    ...
    //generates field values for the entries in the database
